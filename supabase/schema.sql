...
create table public.service_requests (id uuid primary key default gen_random_uuid(), customer_id uuid not null references public.profiles(id) on delete cascade, category text not null, description text not null, postcode text not null, preferred_timing text not null, budget_min numeric(10,2), budget_max numeric(10,2), urgency text not null default 'flexible', status public.request_status not null default 'open', created_at timestamptz not null default now(), updated_at timestamptz not null default now(), constraint service_requests_budget_check check (budget_min is null or budget_min >= 0) and (budget_max is null or budget_max >= 0) and (budget_min is null or budget_max is null or budget_max >= budget_min));
...
create index service_requests_matching_idx on public.service_requests(category, urgency, budget_min, budget_max, created_at desc);
