...
function extractJson(text: string): UnifiedAgentResult | null {
  ...
  for (const candidate of candidates) try {
    const value = JSON.parse(candidate) as Partial<UnifiedAgentResult>; const decision=value.decision; const rawAction=value.action; const specialist=value.specialist;
    if (!decision || !rawAction || !specialist || !decision.intent || !decision.plan || !specialist.plan) continue;
    ...
    if (kind === 'tool') {
      if (!capability || !String(rawAction.arguments?.query || '').trim()) continue;
    } else if (capability !== null) continue;
...
export async function runAgent(message:string,history:Array<{role:'user';content:string}|{role:'assistant';content:string}>,state:AgentState):Promise<UnifiedAgentResult>{
