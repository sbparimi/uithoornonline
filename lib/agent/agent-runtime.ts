...
- If enough information exists, set status=ready and select an executable discovery capability.
- If verified observations already answer the request, do not repeat the same capability unnecessarily.
...
- Keep plans short and executable. Prefer action over clarification.
- Customer-facing reply must be concise and exclusively in the selected language; it may be empty when an external action is selected.
...
const VALID_CAPABILITIES = new Set<NonNullable<AgentAction['capability']>>(['business.search','business.discover','conversation.respond']);
...
function extractJson(text: string): UnifiedAgentResult | null {
  ...
  for (const candidate of candidates) try {
    ...
    if (action.capability !== null && !VALID_CAPABILITIES.has(action.capability)) continue;
    if (action.capability && action.capability !== 'conversation.respond' && !String(action.query || '').trim()) continue;
    ...
    return {decision:{language:decision.language==='en'?'en':'nl',location:decision.location||{municipality:'Uithoorn',postcode:null,source:'default'},intent:{primary:decision.intent.primary,confidence:Number(decision.intent.confidence??0.8)},entities:decision.entities||{},specialist:decision.specialist,task:{type:String(decision.task?.type||'local_help')},plan:normalizePlan(decision.plan)},action:{capability:action.capability,query:typeof action.query==='string'&&action.query.trim()?action.query.trim():null,rationale:String(action.rationale||'').trim().slice(0,500)},specialist:{reply:String(specialist.reply||'').trim(),captured:specialist.captured&&typeof specialist.captured==='object'?specialist.captured:{},nextRequiredSlot,missingSlots,status:specialist.status==='ready'?'ready':'collecting',shouldSearch:Boolean(specialist.shouldSearch),plan:normalizePlan(specialist.plan)}};
