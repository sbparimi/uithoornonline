...
import { executeTool, type ToolRequest } from './tool-gateway';
...
function observationForProviders(capability: string, providers: AgentProvider[], verified: ReturnType<typeof verifyProviderResults>): HarnessObservation { return { id: crypto.randomUUID(), capability, status: verified.passed ? 'success' : 'failed', summary: verified.reason, evidence: verified.evidence, retryable: !verified.passed }; }
...
export async function runHarness(message:string,history:Array<{role:'user'|'assistant';content:string}>,initialState:AgentState,runAgent:(message:string,history:Array<{role:'user'|'assistant';content:string}>,state:AgentState)=>Promise<UnifiedAgentResult>,searchProviders:ProviderSearch):Promise<HarnessResult>{
  ...
  for(let iteration=1;iteration<=MAX_ITERATIONS;iteration+=1){
    ...
    if(tool.status==='failed'){
      ...
      state=applyHarnessState(state,{observations:[...state.harness.observations,{id:crypto.randomUUID(),capability:tool.capability,status:'failed',summary:failure.message,evidence:[],retryable:tool.retryable}]});
    ...
    state=applyHarnessState(state,{observations:[...state.harness.observations,observationForProviders(tool.capability,providers,verification)],nextAction:verification.passed?'verify_results':'recover_search'});
