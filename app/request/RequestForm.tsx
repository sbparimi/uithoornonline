...
export default function RequestForm() {
  ...
  async function submit(e: FormEvent) {
    ...
    if (!response.ok) { setError(data.error === 'invalid_request' ? 'Vul alle velden in. Beschrijf duidelijk wat er moet gebeuren.' : 'Je aanvraag kon niet worden verstuurd. Probeer het opnieuw.'); return; }
    setSuccess(data.id); setDescription(''); setPostcode(''); setPreferredTiming('');
  }

  if (success) return <div className="request-success"><Check /><div><span className="uo-kicker">Aanvraag ontvangen</span><h2>We gaan dit lokaal regelen.</h2><p>Je aanvraag staat klaar voor passende, geverifieerde lokale aanbieders. Je aanvraagnummer is <strong>{success.slice(0, 8).toUpperCase()}</strong>.</p><a className="primary platform-button-link" href="/account">Bekijk mijn aanvragen <ArrowRight /></a></div></div>;
  ...
  return <form className="request-form" onSubmit={submit}>
    ...
    <label>Wanneer moet het gebeuren?<select required value={preferredTiming} onChange={e => setPreferredTiming(e.target.value)}><option value="" disabled>Selecteer een optie</option><option>Vandaag</option><option>Deze week</option><option>Volgende week</option><option>Deze maand</option><option>Flexibel</option></select></label>
