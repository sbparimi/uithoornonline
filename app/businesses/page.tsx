...
export default async function BusinessesPage({ searchParams }: { searchParams: Promise<{ search?: string }> }) {
  ...
  try {
    ...
    const result = await supabase
      .from('businesses')
      .select('id,name,category,description,postcode,website,phone,verified')
      .eq('active', true)
      .order('name');
    if (result.error) {
  ...
  const items = businesses.map((b) => ({
    ...
    title: b.name,
    meta: b.category,
    description: b.description || 'Lokale aanbieder in Uithoorn en De Kwakel.',
    postcode: b.postcode || 'Uithoorn',
    website: b.website,
    phone: b.phone,
    verified: b.verified,
  }));
