...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  if (!configured()) return { title: 'Lokale aanbieder — Uithoorn.online' };
...
export default async function BusinessProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="uo-site uo-directory">
    ...
    <section className="uo-provider-profile"><a className="platform-secondary-link" href="/businesses"><ArrowLeft /> Alle aanbieders</a><div className="uo-provider-profile-head"><div><span className="uo-kicker">{business.category}</span><h1>{business.name}</h1><p className="uo-provider-profile-intro">{business.description || 'Lokale aanbieder in Uithoorn en De Kwakel.'}</p>{business.verified && <div className="uo-provider-trust"><BadgeCheck /> Geverifieerde lokale aanbieder</div>}</div><aside className="uo-provider-profile-card"><h2>Contact</h2>{(business.address || business.postcode) && <div className="uo-provider-detail"><MapPin /><span>{business.address || business.postcode}<br />Uithoorn & De Kwakel</span></div>}{business.phone && <div className="uo-provider-detail"><Phone /><a href={`tel:${business.phone}`}>{business.phone}</a></div>}{business.website && <div className="uo-provider-detail"><Globe /><a href={business.website} target="_blank" rel="noreferrer">Website</a></div>}<div className="uo-provider-profile-actions"><a className="primary" href="/request">Vraag lokale hulp <ArrowRight /></a>{business.phone && <a className="secondary" href={`tel:${business.phone}`}><Phone /> Bel aanbieder</a>}</div><p className="uo-provider-note">Uithoorn.online toont alleen informatie die door een lokale aanbieder is aangeleverd en door het platform is geverifieerd.</p></aside></div></section>
