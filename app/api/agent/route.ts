...
function applyAgentResult(agent: UnifiedAgentResult, previousState: AgentState, safety: AgentState['safety']): { state: AgentState; specialist: UnifiedAgentResult['specialist'] } {
  let state = applyOrchestratorDecision(agent.decision, previousState);
  state.safety = safety;
  state = applySpecialistResult(state, agent.specialist);
  return { state, specialist: agent.specialist };
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    try { agent = await runAgent(requestMessage, history, previousState); }
    catch (error) { console.error('AGENT_RUNTIME_ERROR', error instanceof Error ? error.message : 'unknown_error'); return backendFailureResponse(previousState.language, previousState, safety); }

    const applied = applyAgentResult(agent, previousState, safety);
    let state = applied.state;
    const specialistResult = applied.specialist;
    ...
    const reply = hasProviderResults ? '' : specialistResult.reply;
