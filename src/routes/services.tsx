...
export const Route = createFileRoute("/services")({ component: ServicesPage, head: () => ({ meta: [
  ...
  { name: "description", content: "Vertel welke lokale dienst je zoekt in Uithoorn." },
] }) });
  ...
  return <AppShell>
    ...
    <main className="mx-auto max-w-4xl px-5 py-7 pb-16 sm:px-6 lg:px-8">
      {submitted ? <div className="rounded-2xl border border-border bg-white p-7 text-center shadow-sm"><CheckCircle2 className="mx-auto text-red" size={30}/><h2 className="mt-3 font-serif text-2xl text-navy">Aanvraag ontvangen</h2><p className="mx-auto mt-2 max-w-md text-sm leading-6 text-muted-foreground">Je aanvraag is privé opgeslagen. Dit is nog geen bevestigde afspraak of garantie over een dienstverlener.</p><button onClick={()=>{setSubmitted(false);setSelected(null);setDetails("");setPostcode("");}} className="mt-5 rounded-xl bg-navy px-5 py-3 text-sm font-semibold text-white">Nieuwe aanvraag</button></div> : <>
        ...
        <section className="mt-7 rounded-2xl border border-border bg-white p-5 shadow-sm"><h2 className="font-serif text-xl text-navy">Aanvraag</h2><label className="mt-4 block text-xs font-semibold text-navy" htmlFor="service-details">Wat heb je nodig? <span className="font-normal text-muted-foreground">(optioneel)</span></label><textarea id="service-details" value={details} onChange={e=>setDetails(e.target.value)} rows={4} maxLength={1000} placeholder="Bijv. twee kamers schoonmaken, liefst op een doordeweekse ochtend…" className="mt-1.5 w-full resize-none rounded-xl border border-border bg-cream px-3 py-3 text-sm outline-none"/><label className="mt-4 block text-xs font-semibold text-navy" htmlFor="service-postcode">Postcode <span className="font-normal text-muted-foreground">(optioneel)</span></label><input id="service-postcode" value={postcode} onChange={e=>setPostcode(e.target.value)} placeholder="Bijv. 1421 AB" maxLength={7} className="mt-1.5 w-full rounded-xl border border-border bg-cream px-3 py-3 text-sm outline-none"/><p className="mt-4 text-[11px] leading-5 text-muted-foreground">Uithoorn Online is een onafhankelijk platform. We presenteren geen bedrijf als gecontroleerd, aanbevolen of beschikbaar zonder afzonderlijke verificatie.</p><button onClick={submit} className="mt-5 flex w-full items-center justify-center gap-2 rounded-xl bg-red px-4 py-4 text-sm font-semibold text-red-foreground">Aanvraag bewaren <ArrowRight size={17}/></button></section>
      </>}
    </main>
  </AppShell>;
}
