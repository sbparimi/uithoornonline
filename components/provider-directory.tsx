...
export function ProviderDirectory({ items, initialQuery = '', showDemandFilters = false }: { items: ProviderItem[]; initialQuery?: string; showDemandFilters?: boolean }) {
  const inferredFilter = inferDemandFilter(initialQuery);
  const [query, setQuery] = useState(initialQuery);
  const [filter, setFilter] = useState<string>(inferredFilter);
  ...
  const filtered = useMemo(() => items.filter((item) => {
    ...
    const matchesQuery = !q || inferredFilter !== 'all' || haystack.includes(q);
    ...
    return matchesQuery;
  }), [items, query, filter, inferredFilter]);
