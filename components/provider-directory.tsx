...
function inferDemandFilter(query: string) {
  ...
  if (normalized === 'garden' || normalized.includes('tuin') || normalized.includes('garden')) return 'garden';
  if (normalized.includes('home improvement') || normalized.includes('huis') || normalized.includes('klus')) return 'home improvement';
  if (normalized.includes('plumber') || normalized.includes('loodgieter')) return 'plumber';
  if (normalized.includes('handyman')) return 'handyman';
  return 'all';
}
...
export function ProviderDirectory({ items, initialQuery = '', showDemandFilters = false }: { items: ProviderItem[]; initialQuery?: string; showDemandFilters?: boolean }) {
  ...
  const filtered = useMemo(() => items.filter((item) => {
    ...
    if (filter === 'home improvement') return matchesQuery && /renov|paving|tiling|fenc|klus|onderhoud|bouw|home improvement/.test(haystack);
  ...
  return <main className="uo-directory">
    <header className="uo-header"><div className="uo-header-inner"><a href="/" className="uo-brand"><span className="uo-brand-mark"><img src="/icon.svg" alt="" /></span><span>ithoorn<span>.online</span></span></a><nav className="uo-nav"><a href="/businesses">Ontdek</a><a href="/events">Agenda</a><a href="/deals">Aanbiedingen</a><a href="/food">Indian food</a></nav><div className="uo-header-actions"><span className="uo-location"><MapPin /> Uithoorn & De Kwakel</span><a className="uo-header-cta" href="/signup/account?role=provider">Bedrijf toevoegen</a></div></div></header>
    <section className="uo-directory-hero"><div><span className="uo-kicker">Een plek in Uithoorn</span><h1>Vind wat je zoekt.</h1><p>Ontdek lokale bedrijven, professionals en voorzieningen in Uithoorn en De Kwakel. Zoek op naam, dienst of buurt.</p><div className="uo-directory-search"><Search /><input value={query} onChange={(e) => { setQuery(e.target.value); if (!e.target.value.trim()) setFilter('all'); }} placeholder="Zoek restaurant, winkel, loodgieter, tuin…" aria-label="Zoek in Uithoorn" />{query && <button type="button" onClick={() => { setQuery(''); setFilter('all'); }} aria-label="Wis zoekopdracht"><X /></button>}</div></div></section>
    <section className="uo-directory-results">
      {showDemandFilters && <div className="directory-demand"><div className="directory-demand-head"><div><span className="uo-kicker">Populaire lokale zoekopdrachten</span><h2>Waar zijn mensen naar op zoek?</h2></div><button type="button" onClick={() => setShowFilters((value) => !value)}><SlidersHorizontal /> Filters</button></div><div className={`directory-filter-list ${showFilters ? 'open' : ''}`}>{demandFilters.map(([value, label]) => <button key={value} className={filter === value ? 'active' : ''} type="button" onClick={() => { setFilter(value); setQuery(''); }}>{label}</button>)}</div></div>}
      ...
      {items.length === 0 ? <div className="uo-empty"><MapPin /><h3>Nog geen geverifieerde aanbieders</h3><p>De lokale gids groeit. Meld je bedrijf aan om als eerste zichtbaar te worden.</p><a href="/signup/account?role=provider">Bedrijf aanmelden <ArrowRight /></a></div> : <div className="uo-directory-list">{filtered.map((item) => <article key={item.id ?? item.title}><div className="uo-directory-mark">{item.title.slice(0, 1)}</div><div className="uo-directory-copy"><span>{item.meta} · {item.postcode}{item.verified && <span className="uo-verified"><BadgeCheck /> Geverifieerd</span>}</span><h3>{item.title}</h3><p>{item.description}</p><div className="uo-provider-actions"><a href={profileHref(item)}>Bekijk profiel <ArrowRight /></a>{item.phone && <a href={`tel:${item.phone}`} aria-label={`Bel ${item.title}`}><Phone /></a>}{item.website && <a href={item.website} target="_blank" rel="noreferrer" aria-label={`Website ${item.title}`}><Globe /></a>}</div></div></article>)}</div>}
