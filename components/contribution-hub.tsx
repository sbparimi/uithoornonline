...
export default function ContributionHub() {
  ...
  return <div className="contribution-hub">
    <div className="contribution-options" role="tablist" aria-label="Wat wil je delen?">
      {options.map(({ kind: optionKind, title, text, icon: Icon }) => <button key={optionKind} type="button" role="tab" aria-selected={kind === optionKind} className={kind === optionKind ? 'active' : ''} onClick={() => choose(optionKind)}><Icon /><span><strong>{title}</strong><small>{text}</small></span></button>)}
    </div>

    <div className="contribution-form-wrap">
      <div className="contribution-form-intro"><span className="uo-kicker">{selected.title}</span><h2>Help Uithoorn <em>lokaal</em> sterker te maken.</h2><p>{selected.text} Alles wordt eerst gecontroleerd.</p></div>
      <form className="contribution-form" onSubmit={submit}>
        ...
        <button className="discovery-primary" disabled={state === 'sending'} type="submit">{state === 'sending' ? 'Bezig met indienen…' : 'Bijdrage indienen'} <ArrowRight /></button>
        <small>Je bijdrage wordt eerst gecontroleerd. Alleen goedgekeurde informatie wordt openbaar gepubliceerd.</small>
      </form>
    </div>
  </div>;
}
