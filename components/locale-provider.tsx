...
function tx(value: string, locale: Locale) { if (locale === 'nl') return value; let result = value; for (const key of allKeys) if (result.includes(key)) result = result.split(key).join(allTranslations[key]); return result.replace(/(\d+) resultaten\b/g, '$1 results').replace(/(\d+) resultaat\b/g, '$1 result'); }
