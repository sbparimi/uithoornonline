...
import { ArrowRight, Check, CircleUserRound, MapPin, Menu, Search, Store, Wrench, X } from 'lucide-react';
...
export function HomePage() {
  ...
  const filteredBusinesses = useMemo(
    () => businesses.filter((item) => `${item.name} ${item.type} ${item.desc}`.toLowerCase().includes(normalizedQuery)),
    [normalizedQuery]
  );
  const filteredFood = useMemo(
    () => foodSpots.filter((item) => `${item.name} ${item.type} ${item.highlight}`.toLowerCase().includes(normalizedQuery)),
    [normalizedQuery]
  );
  const filteredWorkshops = useMemo(
    () => workshops.filter((item) => `${item.title} ${item.provider} ${item.meta} ${item.description}`.toLowerCase().includes(normalizedQuery)),
    [normalizedQuery]
  );
  ...
  const goTo = (name: string) => {
    setCategory(name);
    setMobileOpen(false);
    const target = document.getElementById(sectionForCategory[name] ?? 'explore');
  ...
  return <main className="wk-app" id="top">
    <aside className="side-rail">
      ...
      <nav className="side-nav" aria-label="Hoofdnavigatie">
        <button className={category === 'Ontdek' ? 'active' : ''} onClick={() => goTo('Ontdek')}><span className="nav-circle">◉</span>Ontdek</button>
        <button className={category === 'Diensten' ? 'active' : ''} onClick={() => goTo('Diensten')}><Wrench /> Diensten</button>
        <button className={category === 'Workshops' ? 'active' : ''} onClick={() => goTo('Workshops')}><Store /> Workshops</button>
        <button className={category === 'Indian food' ? 'active' : ''} onClick={() => goTo('Indian food')}><span className="nav-circle">◈</span>Indian food</button>
    ...
    <div className="wk-content">
      <header className="topbar">
        ...
        <nav className="category-nav" aria-label="Categorieën">
          {navCategories.map((item) => <button type="button" key={item} className={category === item ? 'active' : ''} onClick={() => goTo(item)}>{item}</button>)}
        </nav>
      ...
      <div className="workspace">
        <section className="feed" id="explore">
          ...
          {hasSearch && <section className="search-summary" aria-live="polite"><div><strong>{resultCount}</strong> lokale resultaten voor <b>“{query}”</b></div><button type="button" onClick={() => setQuery('')}>Toon alles</button></section>}

          {hasSearch && resultCount === 0 && <section className="empty-search"><Search /><h2>Niets gevonden</h2><p>Probeer bijvoorbeeld <button type="button" onClick={() => setQuery('klus')}>klus</button>, <button type="button" onClick={() => setQuery('keramiek')}>keramiek</button> of <button type="button" onClick={() => setQuery('biryani')}>biryani</button>.</p></section>}
