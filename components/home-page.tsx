...
export function HomePage() {
  ...
  return <main className={`uo-home ${chatOpen ? 'uo-home-chat-open' : ''}`} id="top">
    <header className="uo-home-header">
      ...
      <nav className="uo-home-nav" aria-label="Hoofdnavigatie"><a className="active" href="#top">Home</a><a href="/businesses">Bedrijven</a><a href="/events">Evenementen</a><a href="/deals">Aanbiedingen</a><a href="#about">Informatie</a><a href="#about">Over ons</a></nav>
    ...
    <section className="uo-home-hero">
      ...
      <div className="uo-home-hero-inner">
        <div className="uo-home-kicker">ONTDEK · VERKEN · STEUN LOKAAL</div><h1>Alles in Uithoorn,<br /><em>op één plek</em></h1><p>Vind lokale bedrijven, evenementen, aanbiedingen en handige informatie.<br />Ontdek wat er gebeurt in Uithoorn en steun je lokale gemeenschap.</p>
        <form className="uo-home-search" onSubmit={submitSearch} role="search"><Search aria-hidden="true" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Zoek een restaurant, winkel, evenement, dienst..." aria-label="Zoek in Uithoorn" /><button type="submit">Zoeken</button></form>
        <div className="uo-home-popular"><span>Populaire zoekopdrachten:</span>{popularSearches.map((item) => <a key={item.label} href={item.href}>{item.label}</a>)}</div>
      </div><div className="uo-home-handwriting">Lokaal<br />Verbindt<br />Uithoorn <span>♡</span></div>
    </section>

    <section className="uo-home-cards" aria-label="Ontdek Uithoorn">{cards.map(({ title, text, href, icon: Icon, tone }) => <a key={title} href={href} className={`uo-home-card tone-${tone}`}><span className="uo-home-card-icon"><Icon /></span><span className="uo-home-card-copy"><strong>{title}</strong><span>{text}</span></span><span className="uo-home-card-arrow"><ChevronRight /></span></a>)}</section>
